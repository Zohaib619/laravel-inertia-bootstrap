...
<template>
    <div class="flex justify-between ">
        ...
        <Link :href="route('roles.create')" class="p-2 text-sm text-white font-semibold bg-blue-600 hover:bg-blue-800 rounded">Create Role</Link>
    </div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="role in roles" :key="role.id"
                    ...
                    <td class="px-6 py-4 ">
                        <Link :href="route('roles.edit', role.id)" class="font-medium me-1 text-blue-600 dark:text-blue-500 hover:underline">Edit</Link>
                        <Link :href="route('roles.show', role.id)" class="font-medium ms-1 text-gray-600 dark:text-gray-500 hover:underline">Show</Link>
                        <Link :href="route('roles.destroy', role.id)" method="DELETE" as="button" class="font-medium ms-1 text-red-600 dark:text-red-500 hover:underline">Delete</Link>
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <nav aria-label="Page navigation example">
        <ul class="flex items-center -space-x-px h-10 text-base mt-5">
            <li>
                <a href="#"
